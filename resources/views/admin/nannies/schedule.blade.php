...
var unavailabilies = $.parseJSON('<?php echo json_encode($unavailabilies) ?>');

for(i=0; i<Object.keys(unavailabilies).length; i++){

    var start = unavailabilies[i]['start_date'];
    var title = unavailabilies[i]['difference']+' - '+unavailabilies[i]['start_time']+'/'+unavailabilies[i]['end_time'];
    var end = unavailabilies[i]['end_date'];

  events.push({title: 'Unavailable',start: start, end: end, color  : '#ff8080'} );
}
